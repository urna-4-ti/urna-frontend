...
export default function votePresidentPage () {
    ...
    const { data: candidateId, refetch } = useQuery({
		queryKey: ["get-governmentFormId", candidate],
		queryFn: () => getCandidateId(candidate),
		enabled: !!candidate,
	});
    ...
    const handleForm = async (data: votingProps) => {
        ...
        setCandidate(votingNumbers[0].concat(votingNumbers[1]));
    ...
    return(
        <section className="voteSection">
            ...
            {
                showMessage ? <span className="successSpan">Voto em {candidateId!.name} com sucesso <Image src={certifiedIcon} alt="certified" /></span> : null
            }
            {
                showGoOutDiv ? 
