...
const RegimeVote = () => {
  ...
  const voteIn = async () => {
    ...
    const politicalRegimeId = electionData?.politicalRegimes.filter(item => item.cod === Number(`${slotValue1}${slotValue2}${slotValue3}`))
    await mutateAsync({
      votingId: idElection,
      whiteVote: 'false',
      politicalRegimeId: politicalRegimeId[0] ? politicalRegimeId[0] : '',
