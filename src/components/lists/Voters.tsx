...
const Voters = ({ value }: Search) => {
	...
	const filteredVoter = voters?.filter((item) => {
		if (value) {
	...
	return (
		<>
			{!isLoading ? (
				<>
					{filteredVoter !== undefined && filteredVoter.length > 0 ? (
						<>
							{filteredVoter?.map((item) => (
